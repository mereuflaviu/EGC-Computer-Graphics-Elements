...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    GLint loc_material_kd = glGetUniformLocation(shader->program, "material_kd");    // material diffuse light
    glUniform1f(loc_material_kd, materialKd);

    GLint loc_material_ks = glGetUniformLocation(shader->program, "material_ks");	// material specular light
